#!/bin/bash
# deploy-with-chat.sh - Deploy Expense Management App WITH GenAI services
...
echo "============================================"
echo " Expense Management - Full Deploy with Chat"
echo "============================================"
...
# ── 2. Deploy Infrastructure (with GenAI) ────────────────────────────────────
...
# ── 12. Update GenAI Settings File ────────────────────────────────────────────
echo "Step 12: Updating GenAISettings.json..."
cat > GenAISettings.json <<EOF
{
  "OpenAI": {
    "Endpoint": "${OPENAI_ENDPOINT}",
    "DeploymentName": "${OPENAI_MODEL_NAME}"
  },
  "Search": {
...
EOF
echo "  ✓ GenAISettings.json updated"
...
echo "Step 13: Deploying application code..."
az webapp deploy \
  --resource-group "$RESOURCE_GROUP" \
  --name "$APP_SERVICE_NAME" \
  --src-path ./app.zip \
...
echo " The chat UI is powered by Azure OpenAI GPT-4o."
echo ""
